...
Instead, you just need to **put an image (.png, .jpeg or .gif) named `opengraph-image` and/or `twitter-image`** in the same directory of the page that you want the images to be (in the example i want the images in the home, so I placed them in the same level as the `layout.tsx`).
