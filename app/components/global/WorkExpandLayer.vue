<script lang="ts" setup>
import { useCardExpandLayer } from "~/composables/animations/home/useCardExpandLayer";

const { layerRef, coverRef, stageRef, labelRef, roleRef } = useCardExpandLayer();
</script>

<template>
  <div
    ref="layerRef"
    aria-hidden="true"
    class="pointer-events-none invisible fixed inset-0 z-998 flex items-center justify-center"
  >
    <div ref="coverRef" class="bg-background absolute inset-0 z-0 opacity-0" />
    <div ref="stageRef" class="absolute top-1/2 left-1/2 z-1 overflow-hidden" />
    <div
      ref="labelRef"
      class="absolute top-1/2 left-1/2 z-2 flex items-center justify-center opacity-0"
    >
      <span
        ref="roleRef"
        class="text-foreground block max-w-full overflow-hidden text-center font-serif text-base"
      />
    </div>
  </div>
</template>
