...
const Blog: FC = () => {
  ...
  return (
    <div data-testid="homeBlogSection" className="mb-section grid">
      ...
      <div className="col-span-full flex flex-col md:col-start-4 md:col-end-13">
        {posts.map((post: Posts, index: number) => (
          <div key={index}>
            <div className="bg-dark dark:bg-light h-[1px] w-full"></div>
            <Link
              data-testid={`blogPostLink${index}`}
              data-cy="blogPost"
              className="hover:bg-dark dark:hover:bg-light group flex min-h-32 flex-col items-start justify-center gap-small transition md:flex-row md:items-center md:justify-between md:px-small"
              href={`blog/${post?.slug}`}
            >
              <div className="flex flex-col items-start gap-small md:flex-row md:items-center md:gap-medium">
                <span
                  data-cy="blogPostIndex"
                  className="group-hover:text-light darkgroup-hover:text-dark hidden transition md:block"
                >
                  0{index + 1}
                </span>
                <h3
                  data-cy="blogPostTitle"
                  className="group-hover:text-light dark:group-hover:text-dark text-left text-xl font-semibold transition md:text-3xl"
                >
                  {post?.metadata.title}
                </h3>
              </div>
              <div className="flex min-w-fit flex-row gap-smallest md:ml-regular md:flex-col md:text-right">
                <span
                  ...
                  className="group-hover:text-light dark:group-hover:text-dark transition"
                >
